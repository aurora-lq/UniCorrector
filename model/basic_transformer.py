...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, prop, dec_inputs):
        
        enc_outputs, enc_self_attns = self.encoder(enc_inputs, prop)
        dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(dec_inputs, enc_inputs, enc_outputs, prop)
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs, prop):
        ...
        enc_outputs = enc_outputs + type_embeddings  
        print(prop.shape)
        print(enc_inputs.shape)
        assert prop.ndim == enc_inputs.ndim
        enc_inputs = torch.cat([prop, enc_inputs], 1)
        
        type_embd = self.type_emb(torch.zeros((b, 1), dtype=torch.long, device=enc_inputs.device))
        if prop.ndim == 2:
            p = self.prop_nn(prop.unsqueeze(1))  
        else:
            p = self.prop_nn(prop)  # for multiproperty
        p += type_embd
        enc_outputs = torch.cat([p, enc_outputs], 1)
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs,prop):
        ...
        dec_self_attn_subsequence_mask = get_attn_subsequence_mask(dec_inputs).to(
            device)  # [batch_size, tgt_len, tgt_len]以确保解码器不能看到未来的信息
...
class PositionalEncoding(nn.Module):
    def __init__(self, dropout=0.1, max_len=5000):
        ...
        pe = pe.unsqueeze(0).transpose(0, 1)#转换后[max_len,batch,d_model]
